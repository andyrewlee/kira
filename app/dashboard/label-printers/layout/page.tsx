...
export default function LabelPrintersLayoutPage() {
  const key = "lp-editor"
  const listKey = "lp-layouts"
  ...
  const save = () => {
    ...
    const name = prompt("Name this layout", `Label ${layouts.length + 1}`)
    if (!name) return
  ...
  return (
    <div className="space-y-4">
      ...
      {layouts.length === 0 ? (
        <div className="text-sm text-muted-foreground">No saved label layouts yet.</div>
      ) : (
