...
export default function PhoneAgentTemplatesPage() {
  ...
  const useT = (id: string) => { window.location.href = `/dashboard/phone-agent/editor?template=${id}` }
  return (
      ...
      <div className="grid md:grid-cols-2 gap-3">
        {templates.map(t => (
          <div key={t.id} className="border rounded p-3">
            ...
            <button className="mt-2 border px-2 py-1 text-sm" onClick={()=>useT(t.id)}>Use Template</button>
          </div>
        ))}
      </div>
    </div>
