...
export default function CanvasEditor({ storageKey, width, height }: Props) {
  ...
  const addEl = (type: El["type"]) => {
    ...
    switch (type) {
      case "text":
        el = { ...base, type, text: "New Text", fontSize: 14, align: "left" }
        break
      case "image":
  ...
  const insertVar = (token: string) => {
    ...
    updateSel({ text: (sel as TextEl).text + token })
  }
  ...
  return (
    <div className="grid grid-cols-12 gap-4">
      ...
      <div className="col-span-7">
        ...
        <div className="bg-white border shadow-sm p-4 inline-block">
          <div
            ref={canvasRef}
          ...
          >
            {els.map(el => (
              <ElView key={el.id} el={el} selected={el.id === selected} onPointerDown={(e) => startDrag(el.id, e)} />
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="col-span-3 space-y-2">
        ...
        {!sel ? (
          ...
          <TextInspector el={sel as TextEl} onChange={updateSel} />
        ) : sel.type === "image" ? (
...
function ElView({ el, selected, onPointerDown }: { el: El; selected: boolean; onPointerDown: (e: React.PointerEvent) => void }) {
  const common = {
  ...
  if (el.type === "text") {
    ...
    return <div {...common} style={{ ...common.style, padding: 2 }}><div style={{ fontSize: t.fontSize, fontWeight: t.bold ? 700 : 400, textAlign: t.align as any }}>{t.text}</div></div>
  }
  if (el.type === "image") {
...
function TextInspector({ el, onChange }: { el: TextEl; onChange: (p: Partial<TextEl>) => void }) {
  return (
    <div className="space-y-2">
      <textarea className="w-full border rounded p-2 text-sm" rows={4} value={el.text} onChange={e => onChange({ text: e.target.value })} />
