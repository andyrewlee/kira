...
// Enable TURN servers for restrictive networks (env-driven)
const ENABLE_TURN = process.env.ENABLE_TURN === "1";
  ...
  constructor(config: RTCPeerConfig) {
    ...
    if (ICE_SERVERS_JSON) {
      try {
        ...
        console.warn(`[${config.sessionId}] ⚠️ Failed to parse ICE_SERVERS env, using defaults`, err);
      }
    }
